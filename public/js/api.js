...
const AuthAPI = {
  ...
  actualizarPerfil: (body)     => req('/auth/perfil', { method: 'PUT', body: JSON.stringify(body) }),
  desactivar: ()               => req('/auth/desactivar', { method: 'PUT' }),
  eliminar : ()                => req('/auth/eliminar', { method: 'DELETE' }),
};
...
const ReservasAPI = {
  ...
  eliminar       : (id, motivo) => req(`/reservas/${id}?motivo=${encodeURIComponent(motivo || '')}`, { method: 'DELETE' }),
};
