...
const eliminarReserva = async (req, res) => {
  ...
  try {
    const motivo = req.body.motivo || req.query.motivo || 'Cancelada por el usuario';
    const canceladoPor = (req.user?.IDRol === 1 || req.user?.idRol === 1) ? 'Admin' : 'Cliente';
    const result = await reservaService.eliminarReserva(req.params.id, motivo, canceladoPor);
    res.status(200).json({ ok: true, ...result });
  } catch (error) {
    res.status(error.status || 500).json({ ok: false, mensaje: error.message });
  }
